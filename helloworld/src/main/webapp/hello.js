...
function enableButtons () {
	...
	// Update the button label now that the button is active
	btn.value="Click me for a personal and timely greeting";
...
function greetByPeriod () {
	...
	// Shas: Get the period from the time of the client PC
	var period = document.getElementById("period_field").value;

	// Call the sayHelloByName() function.
	// It takes one argument "name"
	// On success, pass the response to sayHelloCallback()
	var request = gapi.client.helloworldendpoints.greetByPeriod({'name': name, 'time': period });
	request.execute(sayHelloCallback);
}
